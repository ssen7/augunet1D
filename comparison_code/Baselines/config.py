...
config["dataset"] = "mice_eeg"  # set to: 'dots', 'movie', 'vss', 'zuco', 'sleep'
...
config["window_len"] = 2000
config["data_dir"] = (
    "/standard/ivy-hip-rderi/ss4yd/DETRtime/Baselines/"
)
config["dice_weights"] = [0.95, 0.05]  # label distribution

### OLD CODE: WRONG! DOES NOT CHANGE THE DICE WEIGHTS
# config["dice_weights"] = (
...
### CORRECT CODE
config["dice_weights"] = (
    np.array(config["dice_weights"]).sum()/np.array(config["dice_weights"])
) 
