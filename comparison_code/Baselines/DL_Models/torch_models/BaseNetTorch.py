...
from DL_Models.torch_models.torch_utils.training import train_loop, validation_loop, test_loop
...
class BaseNet(nn.Module):
    ...
    def __init__(
        ...
        if loss == "dice-loss":
            ...
            # print(f'Min: {(self.get_nb_features_output_layer(), self.nb_outlayer_channels)}') ## DEBUG PRINT STATEMENT
            ...
            self.out_linear = nn.Sequential(
                nn.Linear(2000, self.output_shape),
    ...
    def predict(self, test_dataloader):
        ...
        # validation_loop(test_dataloader, self.float().to('cuda'), self.loss_fn)
        return test_loop(test_dataloader, self.float().to('cuda'))
