...
def train_loop(dataloader, model, loss_fn, optimizer):
    ...
    #
    model.train()
    predictions, correct_pred, ground_truth = [], [], []
    for batch, (X, y) in enumerate(tqdm(dataloader)):
        # Move tensors to GPU
        if torch.cuda.is_available():
            X = X.cuda()
            y = y.cuda()
        # print(model) ## DEBUG PRINT STATEMENT
        pred = model(X)
        y = y.squeeze()
        bs, sq = y.size()
        y_dice = y.long().reshape(-1, 1)
        y_dice = (
            make_one_hot(y_dice, 5) if config["dataset"] == "sleep" else make_one_hot(y_dice, 2)
        )
        if torch.cuda.is_available():
            y_dice = y_dice.cuda()
    ...
    train_scores = classification_report(
        ground_truth, predictions, digits=3, output_dict=True
    )
    # train_loss_epoch, train_acc_epoch, train_f1_weighted 
    train_acc_epoch=train_scores['accuracy']
    train_f1_weighted=train_scores['weighted avg']['f1-score']
    logging.info(f"Avg training loss: {loss:>7f}")
    return float(loss),train_acc_epoch,train_f1_weighted
...
def validation_loop(dataloader, model, loss_fn):
    ...
    print(f"Avg validation loss: {loss:>7f}")
    val_scores = classification_report(
        ground_truth, predictions, digits=3, output_dict=True
    )
    # logging.info(f"Validation performance: \n" f" {val_scores}")
    print(f"Validation performance: \n" f" {val_scores}")
    return float(loss), val_scores['accuracy'], val_scores['weighted avg']['f1-score']


def test_loop(dataloader, model):
    predictions, correct_pred, all_probs = [], [], []
    with torch.no_grad():
        model.eval()
        for batch, (X, y) in enumerate(tqdm(dataloader)):
            # Move tensors to GPU
            if torch.cuda.is_available():
                X = X.cuda()
            pred = model(X)
            pred_dice = pred.reshape(-1, 2)
            probs, pred_indices = torch.max(pred_dice, 1)
            predictions += list(pred_indices.cpu().numpy().flat)
            all_probs += list(probs.cpu().numpy().flat)
            correct_indices = y.long().flatten()
            correct_pred += list(correct_indices.cpu().numpy())
            del X
            del y
            torch.cuda.empty_cache()
    train_scores = classification_report(
        correct_pred, predictions, digits=3, output_dict=True,
    )
    logging.info(f"Test performance: \n" f"{train_scores}")
    print(f"Test performance: \n" f"{train_scores}")
    logging.info(f"Test Confusion matrix: \n{confusion_matrix(correct_pred, predictions)}")
    return np.array(correct_pred), np.array(predictions),np.array(all_probs)
