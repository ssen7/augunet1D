...
def create_dataloader(X, y, batch_size, drop_last=True):
    ...
    # y[0:5, 1]
    if (config["dataset"] != "sleep") & (config["dataset"] != "mice_eeg"):
    ...
    return DataLoader(dataset, batch_size=batch_size, drop_last=drop_last, num_workers=10)
...
def create_biased_dataloader(X, y, batch_size, drop_last=True, seq_length=500):
    ...
    if config["dataset"] != "sleep":
        if config["dataset"] == "mice_eeg":
            y=y # keep the labels as is as they are already binarized.
        else:
            func = np.vectorize(cat_dict.get)
            y = func(y)
    # y=np.vstack(y).astype(np.float32)
    ...
    dataset = TensorDataset(tensor_x, tensor_y)
    return DataLoader(
        dataset, batch_size=batch_size, drop_last=drop_last, num_workers=10, sampler=sampler
    )
